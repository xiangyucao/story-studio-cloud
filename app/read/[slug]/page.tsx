...
export default async function ReaderPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const db = getDb();
  const [user, workRows] = await Promise.all([getChatGPTUser(), db.select().from(works).where(and(eq(works.slug, slug), eq(works.isPublished, true))).limit(1)]);
  ...
  return <div className="reader-page"><SiteHeader user={user} /><main className="reader-wrap"><header className="reader-title"><span>{work.genre} · {work.language}</span><h1>{work.title}</h1><p>{work.description || "作者还没有填写简介。"}</p></header>
    ...
    {!chapterRows.length && <div className="empty-state">作者尚未发布章节。</div>}
  </main><SiteFooter /></div>;
}
