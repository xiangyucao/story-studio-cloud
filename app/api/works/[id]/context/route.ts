...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  let item:unknown;
  if(body.kind==="character") {const name=required(body.name,"name");if(await duplicateCharacterName(id,name))return Response.json({error:"人物姓名不能重复；JSON 人物关系需要用姓名定位"},{status:409});[item]=await db.insert(characters).values({id:createId("chr"),workId:id,name,role:text(body.role),description:text(body.description),personality:text(body.personality),goal:text(body.goal),createdAt:now,updatedAt:now}).returning();}
...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(body.kind==="character") {const name=required(body.name,"name");if(await duplicateCharacterName(id,name,itemId))return Response.json({error:"人物姓名不能重复；JSON 人物关系需要用姓名定位"},{status:409});[item]=await db.update(characters).set({name,role:text(body.role),description:text(body.description),personality:text(body.personality),goal:text(body.goal),updatedAt}).where(and(eq(characters.id,itemId),eq(characters.workId,id))).returning();}
...
async function duplicateCharacterName(workId:string,name:string,excludeId?:string){const rows=await getDb().select({id:characters.id,name:characters.name}).from(characters).where(eq(characters.workId,workId));const key=name.trim().toLocaleLowerCase();return rows.some((row)=>row.id!==excludeId&&row.name.trim().toLocaleLowerCase()===key);}
